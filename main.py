...
async with AsyncSessionLocal() as session:
result = await
session.execute(select(SecurityLog).order_by(SecurityLog.timestamp.des
c()).limit(5))
...
await message.answer(f"🔒 User `{target_id}` added to
shadowban list.", parse_mode="Markdown")
except Exception:
...
await message.answer(f"🔓 User `{target_id}` removed from
shadowban list.", parse_mode="Markdown")
except Exception:
...
if message.from_user.id == config.OWNER_ID:
await message.answer("📢 Send the message (Text or Photo) you
...
async with AsyncSessionLocal() as session:
result = await
session.execute(select(User).where(User.is_active == True))
users = result.scalars().all()
if not users:
return await message.answer("No active users found in the
database.")
...
async with AsyncSessionLocal() as session:
await
session.execute(update(User).where(User.id.in_(dead_users)).values(is_
active=False))
await session.commit()
stats = (
...
user.balance_inr += amount_inr
user.balance_usd += amount_usd
msg = f"✅ Successfully **added** ₹{amount_inr:.2f} to
user `{target_id}`."
else:
user.balance_inr = max(0, user.balance_inr -
amount_inr)
user.balance_usd = max(0, user.balance_usd -
amount_usd)
msg = f"✅ Successfully **removed** ₹{amount_inr:.2f}
from user `{target_id}`."
await session.commit()
await message.answer(msg, parse_mode="Markdown")
try:
if command.command == "addbalance":
await bot.send_message(target_id, f"🎁 **Admin Added
Balance!**\nYour wallet has been credited with
**₹{amount_inr:.2f}**.", parse_mode="Markdown")
else:
await bot.send_message(target_id, f"⚠️ **Balance
Deducted!**\n**₹{amount_inr:.2f}** has been removed from your wallet
by the Admin.", parse_mode="Markdown")
except Exception:
pass
except Exception:
await message.answer("⚠️ **Incorrect Format!**\nUsage:
`/addbalance <user_id> <amount_in_INR>`\nExample: `/addbalance
123456789 500`", parse_mode="Markdown")
...
"Telegram does not allow bots to withdraw Stars directly via
API.\n\n"
"**To withdraw your Stars:**\n"
"1. Open @BotFather on Telegram.\n"
"2. Select your bot and go to 'Monetization' > 'Telegram
Stars'.\n"
"3. Alternatively, go to
[Fragment.com](https://fragment.com/my/bots).\n"
...
**₹{data['price']} / ${data['usd_price']}**\n\nHow would you like to
pay?"
await message.answer(text,
reply_markup=purchase_options_kb(g_id), parse_mode="Markdown")
return
...
await callback.message.edit_text("👇 Select your payment method
for Instant Purchase:", reply_markup=payment_methods_kb("buy", g_id,
"INR"))
...
history = PurchaseHistory(user_id=user.id,
product_name=group['name'], price=group['price'], currency="INR",
method="Wallet", status="Approved")
session.add(history)
await session.commit()
invite_link = await
safe_invite_generator(group['chat_id'], user.id, group['price'],
group['usd_price'])
if invite_link:
await callback.message.answer(f"✅ **Purchase
Successful using Wallet!**\n\n👉 Join here: {invite_link}",
parse_mode="Markdown")
await bot.send_message(config.OWNER_ID, f"️ **Wallet
Sale!** User `{user.id}` bought {group['name']}.")
else:
await callback.answer("❌ Insufficient Wallet Balance.
Please deposit money first.", show_alert=True)
# ==========================================
...
await state.clear()
await callback.message.edit_text("💵 Which currency do you want to
deposit in?", reply_markup=currency_selection_kb())
...
await callback.message.answer(f"Enter the amount in **{currency}
({sym})** you want to deposit (e.g., 500 or 10):",
parse_mode="Markdown", reply_markup=cancel_menu_kb())
...
await state.clear()
await message.answer(f"💵 Deposit Amount:
**{sym}{amount}**\n\nSelect a payment method:",
reply_markup=payment_methods_kb("deposit", str(amount), currency))
except ValueError:
await message.answer("❌ Invalid amount. Please enter a valid
number.")
# ==========================================
...
qr_url =
f"https://api.qrserver.com/v1/create-qr-code/?size=400x400&data={walle
t_address}"
caption = (
f"🪙 **{coin} Payment**\n\n"
f"Amount to Send: **{sym}{target_amount:.2f}**\n\n"
f"Network Address:\n`{wallet_address}`\n\n"
f"Tap the address to copy it. Send the exact amount and click
**✅ I paid**."
)
try:
...
amt = amt_inr
sym = "₹"
await state.update_data(intent=intent, param=param,
currency=currency, method="Amazon", amount=amt)
await state.set_state(PaymentState.waiting_for_amazon_card)
await callback.message.edit_text(f"🎁 **Amazon Gift
Card**\n\nPlease send your **{sym}{amt:.2f}** Amazon Gift Card Code or
Photo in this chat now.", parse_mode="Markdown")
await callback.answer()
...
amt_usd = float(param) if currency == "USD" else (float(param)
/ EXCHANGE_RATE)
title = "Wallet Deposit"
stars_cost = int(amt_usd * 50)
payload =
f"stars_{intent}_{param}_{currency}_{callback.from_user.id}_{stars_cos
t}"
try:
await callback.message.delete()
except Exception:
...
chat_id=callback.message.chat.id, title=title,
description=f"Pay {stars_cost} Stars",
...
[InlineKeyboardButton(text="💬 Contact Admin for UPI Payment",
url=deep_link)],
[InlineKeyboardButton(text="🔙 Back",
callback_data=f"method_back_{intent}_{param}_{currency}")]
])
await callback.message.edit_text(
"To pay via UPI, please contact the Admin. Tap the button
below.",
reply_markup=kb
)
await callback.answer()
...
{data['intent']}"
await bot.send_message(config.OWNER_ID, caption,
parse_mode="Markdown")
await bot.forward_message(config.OWNER_ID, message.chat.id,
message.message_id)
await bot.send_message(config.OWNER_ID, "Approve or Reject?",
reply_markup=admin_approval_kb(message.from_user.id, data['intent'],
data['param'], data['currency'], data['method']))
await message.answer("✅ Gift card received. Your payment has been
forwarded to the Admin for verification.\nPlease wait for approval.",
...
await state.update_data(method=method, intent=intent, param=param,
currency=currency)
await state.set_state(PaymentState.waiting_for_screenshot)
await callback.message.answer("📸 Please send your **Payment
Screenshot** here now.", parse_mode="Markdown",
...
data = await state.get_data()
intent, param, currency, method = data['intent'], data['param'],
data['currency'], data['method']
if intent == "buy":
amt = GROUPS[param]['price'] if currency == "INR" else
GROUPS[param]['usd_price']
else:
amt = float(param)
sym = "₹" if currency == "INR" else "$"
caption = (
...
caption=caption,
reply_markup=admin_approval_kb(message.from_user.id, intent, param,
currency, method), parse_mode="Markdown"
)
await message.answer("✅ **Payment proof sent
successfully.**\nYour payment has been forwarded to the Admin for
verification.\nPlease wait for approval.",
...
_, intent, user_id, param, currency, method =
...
successfully.**\nYour wallet has been credited with
**{sym}{amt:.2f}**.", parse_mode="Markdown")
elif intent == "buy":
group = GROUPS[param]
history = PurchaseHistory(user_id=user.id,
product_name=group['name'], price=group['price'], currency="INR",
...
group['usd_price'])
if invite_link:
await bot.send_message(user_id, f"✅ **Payment
...
group = GROUPS[param]
async with AsyncSessionLocal() as session:
history = PurchaseHistory(user_id=user_id,
product_name=group['name'], price=group['price'], currency="INR",
method=method, status="Rejected")
session.add(history)
await session.commit()
await bot.send_message(user_id, "❌ **Your payment could not be
verified.**\nPlease contact support or upload a valid payment proof.",
parse_mode="Markdown")
await callback.message.edit_reply_markup(reply_markup=None)
...
_, intent, param, currency, user_id, stars =
payload.split("_")
async with AsyncSessionLocal() as session:
user = await get_user(session, int(user_id))
if intent == "deposit":
amt_usd = float(param) if currency == "USD" else
float(param) / EXCHANGE_RATE
...
await message.answer(f"⭐️ **Stars Deposit
Successful!**\nWallet credited with **{sym}{amt:.2f}**.",
parse_mode="Markdown")
elif intent == "buy":
group = GROUPS[param]
history = PurchaseHistory(user_id=user.id,
product_name=group['name'], price=group['price'], currency="INR",
method="Stars", status="Approved")
session.add(history)
await session.commit()
invite_link = await
safe_invite_generator(group['chat_id'], user.id, group['price'],
group['usd_price'])
if invite_link:
await message.answer(f"⭐️ **Stars Payment
Successful!**\nJoin here:\n👉 {invite_link}", parse_mode="Markdown")
# ==========================================
# 🚀 INITIALIZATION
# ==========================================
